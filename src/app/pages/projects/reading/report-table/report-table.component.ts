...
@Component({
  selector: 'tc-reading-report-table',
  templateUrl: './report-table.component.html',
  styleUrls: ['./report-table.component.scss'],
})
export class ReadingReportTableComponent implements OnInit, OnChanges, OnDestroy {
  ...
  async getReports(): Promise<any> {
    return this.http.get<any[]>(`${environment.apiUrl}/api/project/grade/reading/reports`)
        .toPromise()
  ...
  // getAvgReport(semester: number, report: any): number {
  //   let result = 0;
  //   let count = 0;
  //   const st = semester === 1 || !semester ? 1 : 2;
  //   this.semester[st].forEach(item => {
  //     if (
  //         report[item['value'].toString()]
  //         && report[item['value'].toString()].hasOwnProperty('gradesAvg')
  //     ) {
  //       result += Number(report[item['value'].toString()]['gradesAvg']);
  //       count++;
  //     }
  //   });
  //   if (this.semester[st].length) {
